...
namespace Microsoft.NodejsTools.Parsing
{
    ...
    internal class JSParser
    {
        ...
        {
            var classSpan = _curSpan;

            GetNextToken();
            if (_curToken != JSToken.Identifier)
            {
                string identifier = CanBeIdentifier(_curToken);
                // verify that class isn't an identifier - ParseIdentifierInitializer
                ReportError(JSError.NoIdentifier);
            }

            var className = m_scanner.Identifier;

            // extends statement
            GetNextToken();

            Expression leftHandExpression;
            if (_curToken == JSToken.Extends)
            {
                leftHandExpression = ParseLeftHandSideExpression(false);
            }

            if (_curToken != JSToken.LeftCurly)
            {
            ...
            // ParseClassElementList
                ...
                GetNextToken();
            List<Statement> members = new List<Statement>();
            do
            {
                switch (_curToken)
                {
                    case JSToken.Identifier:
                        // PropertyName
                        var functionSpan = _curSpan;
                        var functionName = ParseBindingIdentifier();
                        ...
                        ParseStrictFormalParameters(out formalParameters, out paramSpan);
                        ...
                        var body = ParseFunctionBody2(ref functionSpan, formalParameters, false, paramSpan, /*generatorSpan*/-1, functionName);
                        members.Add(new MethodDefinition(EncodeSpan(functionSpan), formalParameters, body, EncodeSpan(paramSpan), functionName));
                        break;
                }

            } while (_curToken != JSToken.RightCurly);
            classSpan = classSpan.UpdateWith(_curSpan);
            GetNextToken();
            return new ClassDefinition(EncodeSpan(classSpan), className, members);
        }
        ...
        private FunctionObject ParseFunction(FunctionType functionType, IndexSpan fncSpan) {
            ...
            try
            {
                ...
                body = ParseFunctionBody(ref fncSpan, ref formalParameters, inExpression, ref paramsSpan, generatorIndex, name);
        ...
        private Block ParseFunctionBody(ref IndexSpan fncSpan, ref List<ParameterDeclaration> formalParameters, bool inExpression, ref IndexSpan paramsSpan, int generatorIndex, Lookup name)
        {
            ...
            try
            {
                ...
                if (_curToken == JSToken.LeftParenthesis)
                {
                    ParseStrictFormalParameters(out formalParameters, out paramsSpan);
                    fncSpan = fncSpan.UpdateWith(_curSpan);
                    GetNextToken();
                }

                body = ParseFunctionBody2(ref fncSpan, formalParameters, inExpression, paramsSpan, generatorIndex, name);
            }
            finally
            {
                // restore state
        ...
        private Block ParseFunctionBody2(ref IndexSpan fncSpan, List<ParameterDeclaration> formalParameters, bool inExpression, IndexSpan paramsSpan, int generatorIndex, Lookup name)
        {
            Block body;
        ...
        private void ParseStrictFormalParameters(out List<ParameterDeclaration> formalParameters, out IndexSpan paramsSpan)
        {
        ...
        // TODO ParseAssignmentExpression
